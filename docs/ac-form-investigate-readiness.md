...
> **2026-08-24 — ส่วนที่ 2 ถูกซ่อนไว้ชั่วคราว** (`SHOW_INVESTIGATE_SECTION = false`) ใช้การแนบ "เอกสารสอบสวน" + ปุ่มปิดเคสแทน โค้ดและ schema ทั้งหมดยังอยู่ครบ เปิดกลับมาได้ทันที — ดูหัวข้อ [พักการใช้ฟอร์มสอบสวน](#พักการใช้ฟอร์มสอบสวน--ใช้แนบเอกสารแทน-2026-08-24)
...
### 1. ซ่อนส่วนที่ 2 ด้วย feature flag — ไม่ได้ลบโค้ด

[components/ACForm.tsx](../components/ACForm.tsx)

```ts
const SHOW_INVESTIGATE_SECTION = false;   // เปิดกลับมาแค่เปลี่ยนเป็น true
const CASE_CLOSED_STATUS = "Completed Investigate";
```

flag คุม 3 อย่างพร้อมกันผ่าน `hasInvestigateSection`:

| ผลของ flag | รายละเอียด |
|---|---|
| กระดาษแผ่นที่ 2 | ไม่เรนเดอร์ `ACInvestigateComponent` และ FormActionBar ของมัน |
| ปุ่มสลับมุมมอง split/stack | ซ่อนตามไปด้วย (ไม่มีแผ่นที่ 2 ให้วางคู่แล้ว) |
| การยิง API | `loadInvestigate()` + `loadMeasureFiles()` ถูก `return` ตั้งแต่ต้น ไม่ยิงเปล่า |

ใบพิมพ์ไม่ต้องแก้ — `generateACInvestigateHTML()` มี `hasContent` guard อยู่แล้ว ไม่มีข้อมูลก็คืน `''` ส่วนที่ 2 จึงหายจากใบพิมพ์เอง

**โค้ดส่วนที่ 2 ยังอยู่ครบทุกไฟล์** ทั้ง `ACInvestigate.tsx`, proxy, backend, migration ที่รันไปแล้ว — เปิดกลับมาใช้ได้ทันทีโดยไม่ต้องทำอะไรเพิ่ม
